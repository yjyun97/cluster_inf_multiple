...
load("RData/known_dep_psigmaJ_farthest_K20_g1_delta0.RData")
s1_psigmaJ_g1 = vec_p
load("RData/known_dep_psigmaJ_farthest_K20_g3_delta0.RData")
s1_psigmaJ_g3 = vec_p
load("RData/known_dep_psigmaJ_farthest_K20_g5_delta0.RData")
s1_psigmaJ_g5 = vec_p
# setting 1, p_{sigma}
...
s1_psigma_g3 = vec_p
load("RData/known_dep_psigma_farthest_K20_g5_delta0.RData")
s1_psigma_g5 = vec_p
# setting 2, p_{sigma, J}
load("RData/known_dep_psigmaJ_closest_K20_g1_delta0.RData")
s2_psigmaJ_g1 = vec_p
load("RData/known_dep_psigmaJ_closest_K20_g3_delta0.RData")
s2_psigmaJ_g3 = vec_p
load("RData/known_dep_psigmaJ_closest_K20_g5_delta0.RData")
s2_psigmaJ_g5 = vec_p
# setting 2, p_{sigma}
...
s2_psigma_g3 = vec_p
load("RData/known_dep_psigma_closest_K20_g5_delta0.RData")
s2_psigma_g5 = vec_p

##------------------------------------------------------------------------------
## Creates plot setting 1, p_{sigma, J}

df_s1_psigmaJ = data.frame(p = c(s1_psigmaJ_g1, s1_psigmaJ_g3, s1_psigmaJ_g5), 
                           case = as.factor(rep(c("1", "3", "5"), 
                                                each = num_trial)))
df_s1_psigmaJ$case = factor(df_s1_psigmaJ$case, levels = c("1", "3", "5"))
#---
...
plot_s1_psigmaJ = ggplot(df_s1_psigmaJ, aes(sample = p, color = case)) + 
  ...
  xlim(c(0, 1)) + 
  ylim(c(0, 1)) + 
  scale_color_manual(values = c("1" = "#ef476f",
                                "3" = "#ffd166", 
                                "5" = "#26547c")) + 
  plot_details + 
...
df_s1_psigma = data.frame(p = c(s1_psigma_g1, s1_psigma_g3, s1_psigma_g5), 
                          case = as.factor(rep(c("1", "3", "5"), 
                                               each = num_trial)))
df_s1_psigma$case = factor(df_s1_psigma$case, levels = c("1", "3", "5"))
...
df_s2_psigmaJ = data.frame(p = c(s2_psigmaJ_g1, s2_psigmaJ_g3, s2_psigmaJ_g5), 
                           case = as.factor(rep(c("1", "3", "5"), 
                                                each = num_trial)))
df_s2_psigmaJ$case = factor(df_s2_psigmaJ$case, levels = c("1", "3", "5"))
#---
# Citations: the codes below, as well as the lines followed by #---
# for adding two vertical titles for the y-axis is adapted from 
# https://stackoverflow.com/questions/74534630/ggplot2-two-different-font-
...
df_s2_psigma = data.frame(p = c(s2_psigma_g1, s2_psigma_g3, s2_psigma_g5), 
                          case = as.factor(rep(c("1", "3", "5"), 
                                               each = num_trial)))
df_s2_psigma$case = factor(df_s2_psigma$case, levels = c("1", "3", "5"))
...
to_save = paste0("Figures/figure_6.png")
